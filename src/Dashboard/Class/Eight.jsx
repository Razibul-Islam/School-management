...
const Eight = () => {
    ...
    useEffect(() => {
        const url = 'https://school-server-razibul-islam.vercel.app/get-student?class_id=8';
        fetch(url)
            .then(res => res.json())
    ...
    return (
        <div className='p-5'>
            <h1 className='text-center text-xl my-5'>অষ্টম শ্রেণির ছাত্র-ছাত্রীদের তালিকা</h1>
            ...
            <div className={`relative z-10 ${modalTwo}`} aria-labelledby="modal-title" role="dialog" aria-modal="true">
                <div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"></div>
                <div className="fixed inset-0 z-10 overflow-y-auto">
                    <form onSubmit={handleSubmitEdit} className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                        <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4 grid grid-cols-2 gap-3">
                                ...
                                <select name='class' className='block my-3 focus:outline-none border w-full p-2'>
                                    ...
                                    <option selected value="অষ্টম">অষ্টম</option>
                                </select>
