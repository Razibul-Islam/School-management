...
const StudentCabinet = () => {
  ...
  const handleEditCabinet = (_id) => {
    
    const url = `http://localhost:5000/update-cabinet?_id=${_id}`;
    fetch(url, {
      method: "DELETE",
    })
      .then((res) => res.json())
  ...
  }, [handleCabinet, handledeleteCabinet]);
          ...
          <tbody>
            {cabinet.map((singleCabinet, i) => (
              <tr key={i}>
                ...
                <td>
                  <button
                    onClick={() => handleEditCabinet(singleCabinet._id)}
                    className="bg-white rounded-3xl text-red-500"
                  >
                    <EditNoteIcon className="p-1 scale-150" />
                  </button>
                  <button
